...
- **Exchange rates would change constantly in production.** The sample data has a single static rate per currency, but in a real environment rates fluctuate daily. I built `snp_raw__exchange_rates` as a dbt snapshot using the `check` strategy, which implements SCD Type 2 tracking — every time a rate changes, the old row gets a `dbt_valid_to` timestamp and a new row is inserted. This builds a history of rates with validity windows automatically. The `convert_to_usd` macro supports both current-rate and point-in-time conversion (joining on the rate that was valid when the order was placed). With the current static data both modes produce identical results, but the design is production-ready for when rates start moving.
- **Marts are designed to feed a BI tool like Lightdash.** Column-level `meta.dimension` and `meta.metrics` blocks in `_orders.yml` and `_customers.yml` define governed metric definitions (e.g. `repeat_purchaser_rate`, `new_customer_rate`, `avg_days_to_second_purchase`) that a BI layer can pick up automatically. This means metric logic is version-controlled in the dbt repo rather than defined ad-hoc in dashboard SQL, ensuring consistency across all reporting.
- **One orphaned order is kept but cleaned.** One `user_id` in `raw.orders` is the literal string `"None"` — a source system bug where Python `None` was serialized as a string. Staging converts this to SQL `NULL` via `nullif()` (data cleaning — fixing a broken representation, not a business decision). The order itself is preserved in the mart via a left join, with null user dimensions, so it doesn't silently disappear. A `warn`-severity relationship test alerts the team to the source issue. The order stays for completeness but won't corrupt user-level aggregations since `group by user_id` naturally isolates nulls.
